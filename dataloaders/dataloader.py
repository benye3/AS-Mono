...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            image_path = os.path.join(data_path, sample_path.split()[0]) # './NYU_Depth_V2_sync' , 'data/nyu2_test/00008_colors.png'
            image = np.asarray(Image.open(image_path), dtype=np.float32) / 255.0
 
            if self.mode == 'online_eval':
                gt_path = self.args.gt_path_eval 
                depth_path = os.path.join(gt_path, sample_path.split()[1]) # './NYU_Depth_V2_sync' , 'data/nyu2_test/00008_depth.png'
            ...
            print("self.args.do_kb_crop:",self.args.do_kb_crop)
            if self.args.do_kb_crop is True: # False
