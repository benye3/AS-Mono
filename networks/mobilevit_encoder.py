...
from networks.model_config import get_config
...
class MobileViT(nn.Module):
    ...
    def forward(self, input_image):
        ...
        x = self.conv_1(x)
        self.features.append(self.layer_1(x))
        self.features.append(self.layer_2(self.features[-1]))
        self.features.append(self.layer_3(self.features[-1]))
        self.features.append(self.layer_4(self.features[-1]))
        x = self.layer_5(self.features[-1])
        self.features.append(self.conv_1x1_exp(x))
        return self.features


def mobile_vit_xx_small(pretrained_weights_dir="", num_classes: int = 1000):
    ...
    m = MobileViT(config)
    pretrained_weights_path = os.path.join(pretrained_weights_dir, "mobilevit_xxs.pt")
    if pretrained_weights_dir != "":
        assert os.path.exists(pretrained_weights_path), "weights file not exist."
        weights_dict = torch.load(pretrained_weights_path, map_location="cpu")
        weights_dict = weights_dict["model"] if "model" in weights_dict else weights_dict

        for k in list(weights_dict.keys()):
            if "classifier" in k:
                del weights_dict[k]
        m.load_state_dict(weights_dict, strict=False)
    # self.num_ch_enc = [32, 64, 96, 128, 640]  # s channel
    # self.num_ch_enc = [32, 48, 64, 80, 384]  # xs channel
    m.num_ch_enc = [16, 24, 48, 64, 320]  # xxs channel
    return m


def mobile_vit_x_small(pretrained_weights_dir="", num_classes: int = 1000):
    # pretrain weight link
    # https://docs-assets.developer.apple.com/ml-research/models/cvnets/classification/mobilevit_xs.pt
    config = get_config("x_small")
    m = MobileViT(config)
    pretrained_weights_path = os.path.join(pretrained_weights_dir, "mobilevit_xs.pt")
    if pretrained_weights_dir != "":
        assert os.path.exists(pretrained_weights_path), "weights file not exist."
        weights_dict = torch.load(pretrained_weights_path, map_location="cpu")
        weights_dict = weights_dict["model"] if "model" in weights_dict else weights_dict
        ...
        m.load_state_dict(weights_dict, strict=False)
    # self.num_ch_enc = [32, 64, 96, 128, 640]  # s channel
    m.num_ch_enc = [32, 48, 64, 80, 384]  # xs channel
    # self.num_ch_enc = [16, 24, 48, 64, 320]  # xxs channel
    return m


def mobile_vit_small(pretrained_weights_dir="", num_classes: int = 1000):
    # pretrain weight link
    # https://docs-assets.developer.apple.com/ml-research/models/cvnets/classification/mobilevit_s.pt
    config = get_config("small")
    m = MobileViT(config)
    pretrained_weights_path = os.path.join(pretrained_weights_dir, "mobilevit_s.pt")
    if pretrained_weights_dir != "":
        assert os.path.exists(pretrained_weights_path), "weights file not exist."
        weights_dict = torch.load(pretrained_weights_path, map_location="cpu")
        weights_dict = weights_dict["model"] if "model" in weights_dict else weights_dict
        ...
        for k in list(weights_dict.keys()):
            if "classifier" in k:
                del weights_dict[k]
        m.load_state_dict(weights_dict, strict=False)
    m.num_ch_enc = [32, 64, 96, 128, 640]  # s channel
    # self.num_ch_enc = [32, 48, 64, 80, 384]  # xs channel
    # self.num_ch_enc = [16, 24, 48, 64, 320]  # xxs channel
    return m
